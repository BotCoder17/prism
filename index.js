...
client.on("message", (msg, err) => {
  ...
  */
  ...
  let commands_and_prefix = nonne[0],
    mentions = nonne[1],
    pf2 = "",
    cmd = "";
  try {
    for (let i = 0; i < commands_and_prefix.length; i++) {
      ...
      if (
        ...
        yui != "!" &&
        yui != " " &&
        (yui.charCodeAt(0) >= 48 && yui.charCodeAt(0) <= 57)
      ) {
  ...
  for (let i = commands_and_prefix.length; i < mg.length; i++)
    arg1 += mg.charAt(i);
  for (let i = 0; i < ln; i++) pf2 += commands_and_prefix.charAt(i); // get prefix
  for (let i = ln; i < commands_and_prefix.length; i++)
    cmd += commands_and_prefix.charAt(i); // get commands
